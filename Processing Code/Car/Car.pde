...
class CarController {
  ...
  CarController(PVector position, PVector MAX_ACCELERATION, float friction) {
    ...
    this.MAX_ACCELERATION = MAX_ACCELERATION;void drawArrow(float x, float y, float angle) {
  pushMatrix();
    translate(x, y);
    rotate(-angle);
    triangle(-2.5, 5, 0, 0, 2.5, 5);
...
void loadingGame() {
  car1.setImage("https://github.com/AchcarLucas/Engenharia-F-sica-USP-/blob/master/Processing%20Code/Car/car.png?raw=true");
  loading = false;
}
...
void draw() {
  ...
  if(loading) {
    ...
    textSize(30);
    textAlign(CENTER);
    text("Carregando Sprite ...", width / 2, height / 2);
    return;
  }
